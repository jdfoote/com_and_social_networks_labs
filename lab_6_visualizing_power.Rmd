...
Let's look at different measures of power in the Zachary karate network. There are a few new things going on here, so I'll walk through each line:

* `activate(nodes) %>% mutate(name = 1:34)` adds a column called `name` to the nodes, which consists of the number 1 to 8.
...
Note that there is a `mode` parameter that can be either 'out' for only outgoing distance, 'in', for incoming distance, or 'all', which we use, which ignores whether an edge is incoming or outgoing.
...
Why does Node 12 have higher eigenvector centrality than Node 17? What does eigenvector centrality measure about power that other measures miss?
