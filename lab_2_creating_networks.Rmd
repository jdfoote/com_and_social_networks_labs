...
Save the spreadsheet to your computer as an XLSX file called `edges.xlsx` and then:

1. Open `Files` in the bottom right pane, and then `Upload` the file
2. Click `File > Import Dataset > From Excel`
3. Browse to the file
4. At the top of the screen under `File/URL` will be the path to the file. It should look something like `~/Desktop/edges.xlsx`. Copy the path to your clipboard and click `Cancel`.
5. Uncomment the code below (remove the `#`) and paste the file path where my code says `~/your/path/here`.

```{r}
#edgelist <- read_excel("~/your/path/here")
```

You might get an error here that says something like `there is no package called 'readxl'`. If you get that, then you need to run `install.packages('tidyverse')` in the console (this is in the bottom left window). It will take a while to install, but if it works correctly your code above should work.

To test if everything worked right, uncomment the following code to print out all of the edges in R.
...
Unfortunately, it's a little bit trickier to install this package than other packages. You will need to run `install.packages("remotes")` in the Console (at the bottom left), and when that is done run `remotes::install_github("schochastics/networkdata")`, also in the console.
